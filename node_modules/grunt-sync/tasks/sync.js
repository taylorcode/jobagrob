  ...
  grunt.registerMultiTask('sync', 'Synchronize content of two directories.', function() {
    ...
    promise.all(this.files.map(function(fileDef) {
      ...
      return promise.all(fileDef.src.map(function(src) {
        ...
        // if file is a directory and that directory has no files in it that we want from the source
        if(grunt.file.isDir(filePath)) {
            grunt.file.recurse(filePath, function (file) {
              // there is a file in this directory, but we are not sure if it is one that we want
              file = cwd ? file.split(cwd + '/')[1] : file;

              if(fileDef.src.indexOf(file) !== -1) notEmptyDir = true;
        ...
        if(notEmptyDir || isFile) processPair(path.join(cwd, src), dest);
